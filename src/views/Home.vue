<template>
  <v-container fluid>
    <v-row class="align-start">
      <v-col cols="3" v-for="(scene, index) of scenes" :key="index">
        <v-card style="height: 300px">
          <div class="d-flex justify-center align-center" style="height: 300px">
            <div>
              <div class="subtitle-1 text-center font-weight-bold">{{ scene.name }}</div>
              <div class="caption grey--text text-center text-wrap">修改时间{{ scene.update_time }}</div>
            </div>
          </div>
        </v-card>
      </v-col>
      <v-col cols="3">
        <v-card style="height: 300px">
          <div class="d-flex justify-center align-center dark" style="height: 300px">
            <div>
              <v-dialog
                v-model="dialog"
                persistent
                ...
                <v-card dark max-height="600" elevation="2">
                  ...
                  <v-row style="height: 400px" class="py-0">
                    ...
                    <v-col cols="7" class="ml-0 pl-0">
                      <v-card class="mx-auto overflow-y-auto"
                              height="380"
                              tile>
                        <v-list dark v-for="(item, index) of tabsMapping[types]" :key=uniqueKey(index)>
                          <v-list-item>
                            <v-text-field
                              hide-details
                              class="yellow"
                            ...
                            <v-list-item-action>
                              <v-btn icon @click="removeItem(index)">
                                <v-icon color="grey lighten-1">fa-minus-circle</v-icon>
                              </v-btn>
                            </v-list-item-action>
                          </v-list-item>
                        </v-list>
...
export default {
  ...
  methods: {
    ...
    fetchScenes() {
      this.axios.get('/?project_id=1', {headers: {'api_id': '2'}}).then(res => {
    ...
    CreateScenes() {
      ...
      this.dialog = false
      this.axios.post('/', payload, {headers: {'api_id': '2'}}).then(res => {
